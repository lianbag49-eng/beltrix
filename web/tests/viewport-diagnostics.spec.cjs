...
test('record mobile viewport geometry before and after resize',async({page},info)=>{
 ...
 await page.setViewportSize({width:390,height:844});await page.goto('/web/#markets');await expect(page.locator('#fastOrderBar')).toBeVisible();await expect(page.locator('.funding-bar [data-usdt-open]')).toBeVisible();
