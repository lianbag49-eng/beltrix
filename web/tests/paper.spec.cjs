...
test('fund practice accounts and complete futures without any wallet signature',async({page})=>{
 ...
 await page.goto('/web/#markets');await page.locator('#cleanMore').click();await page.locator('[data-clean-route=swap]').click();
 ...
 await page.reload();await page.locator('#cleanMore').click();await page.locator('[data-clean-route=swap]').click();await expect(page.locator('#paperHistory')).toContainText('Net PnL');
